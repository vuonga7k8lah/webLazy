...
class NewsModel
{
    public static function insertData($data)
    {
        return DB::makeConnection()
            ->query("INSERT INTO `tintuc`(`idTinTuc`, `idLoaiTin`, `TomTat`, `TieuDe`, `NoiDung`, `Anh`,`date_time`) VALUES (null,'" .
                $data['idLoaiTin'] . "','" . $data['TomTat'] . "','" . $data['TieuDe'] . "','" . $data['NoiDung'] .
                "','" . $data['Anh'] . "',null)");
    }
    ...
    {
        return DB::makeConnection()->query("SELECT * FROM tintuc ORDER BY idTinTuc ASC limit " . $offset . ",6 ")
            ->fetch_all();
    }

    public static function selectId($id)
    {
        $query = DB::makeConnection()->query("SELECT * FROM tintuc where idtintuc=" . $id);
    ...
    {
        return DB::makeConnection()->query("UPDATE `tintuc` SET `idLoaiTin`='" . $data['idLoaiTin'] . "',`TieuDe`='" .
            $data['TieuDe'] . "',`TomTat`='" . $data['TomTat'] . "',`NoiDung`='" . $data['NoiDung'] . "',`Anh`='" .
            $data['Anh'] . "' WHERE idtintuc='" . $data['idtintuc`'] . "'");
    }

    public static function selectView($id)
    {
        $query = DB::makeConnection()->query("SELECT views FROM `tintuc` WHERE idTinTuc=" . $id);
        return !empty($query) ? $query->fetch_assoc() : [];
    }
