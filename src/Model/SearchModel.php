...
class SearchModel
{
    ...
    {
        $query = DB::makeConnection()->query("SELECT MaSP,TenSP,GiaBan,ChiTiet,Anh FROM sanpham where  TenSP like '%"
            . $like . "%'");
    ...
    {
        return DB::makeConnection()->query("SELECT distinct MaSP FROM donhangphu1 WHERE TrangThai='Thành Công'")
            ->fetch_all();
    }

    public static function FeaturedProducts($id)
    {
        return DB::makeConnection()->query("SELECT MaSP,TenSP,GiaBan,ChiTiet,Anh FROM sanpham where MaSP=" . $id . "")
            ->fetch_all();
    }
}
